  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4 p-6">
      ...
      <textarea
        ...
        rows={6}
        className="lumi-field min-h-36 w-full resize-none focus:border-pink-500/50"
      />
      <button
        onClick={() => send(`作为品牌设计师，为以下品牌提供完整的品牌策略和视觉识别方案：\n\n${input}`)}
        disabled={loading || !input.trim()}
        className="lumi-button-primary border-pink-400/25 bg-pink-500/15 px-6 py-3 text-pink-200 hover:bg-pink-500/25"
      >
        <Palette size={16} />
        {loading ? ui('设计中...', 'Designing...') : (t.designBrandGenerate || ui('生成方案', 'Generate Proposal'))}
      </button>
      {result && (
        <div className="lumi-panel custom-scrollbar max-h-[520px] overflow-y-auto p-4 text-sm whitespace-pre-wrap text-white/80">{result}</div>
      )}
    </div>
  );
}
    ...
    <div className="mx-auto max-w-3xl space-y-4 p-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <input
          ...
          className="lumi-field col-span-2 focus:border-pink-500/50"
        />
      </div>
      <button
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4 p-6">
      ...
      <div className="flex gap-2 mb-2">
        {['Material Design 3', 'Human Interface', 'Ant Design', ui('自定义规范', 'Custom Spec')].map(sys => (
          <button
            ...
            className="lumi-button h-8 border-white/10 px-3 text-xs hover:border-pink-500/30"
          >
            {sys}
          </button>
        ))}
      ...
      <textarea
        ...
        rows={8}
        className="lumi-field min-h-48 w-full resize-none font-mono text-sm focus:border-pink-500/50"
      />
      <button
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4 p-6">
      ...
      <input
        ...
        className="lumi-field w-full focus:border-pink-500/50"
      />
      <button
