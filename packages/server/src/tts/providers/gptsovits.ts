...
const SEGMENTS_DIR = path.join(getDataDir(), 'voice_training', 'segments');
...
export async function synthesizeSpeech(
  ...
  if (voiceId && voiceId.startsWith('gptsovits:')) {
    ...
    if (match) {
      ...
      refAudioPath = refs.length > 0 ? refs[0].path : path.join(getDataDir(), 'voice_training', 'segments', 'segment_0000.wav');
    ...
    refAudioPath = path.join(getDataDir(), 'voice_training', 'segments', 'segment_0000.wav');
